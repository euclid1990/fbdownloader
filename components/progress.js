...
function ProgressBar (options) {
  ...
  this.done = ['⣾', '⣽', '⣻', '⢿', '⡿', '⣟', '⣯', '⣷']
  this.doneTime = this.done.length
...
ProgressBar.prototype.setSchema = function (schema, refresh) {
  this.schema = schema || ' :done [:bar] :current/:total :percent :elapseds :etas'
...
ProgressBar.prototype.compile = function (tokens) {
  ...
  // pre check complete to define [done]
  var doneLen = this.done.length
  if (this.current >= this.total) {
    output = output.replace(/:done/g, '✔')
  } else {
    output = output.replace(/:done/g, this.done[this.doneTime++ % doneLen])
  }
